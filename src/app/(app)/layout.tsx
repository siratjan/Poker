...
import { ConnectionProvider } from '@/components/app/ConnectionProvider';
    ...
    <ConnectionProvider>
      <div className="flex min-h-full flex-1 flex-col">
        <header className="sticky top-0 z-20 border-b border-black/10 bg-[var(--background)]/95 backdrop-blur dark:border-white/10">
          ...
          {/* One banner for the whole app: no network, or a dead live channel
              (WP9, step 2). Inside the sticky header so it never scrolls away. */}
        ...
        <ToastProvider>
          {/* The bottom padding clears the fixed tab bar plus the home
              indicator of a notched phone (WP9, step 5). */}
          <main
            className="mx-auto w-full max-w-2xl flex-1 px-4 pt-4"
            style={{ paddingBottom: 'calc(var(--tabbar-height) + var(--safe-bottom) + 1.5rem)' }}
          >
            {children}
          </main>
        </ToastProvider>
